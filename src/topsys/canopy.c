...
/* Rs_to_Rn: convert incoming shortwave radiation to net radiation (was in init_s.c) */
double Rs_to_Rn(double Rs)
{
...
/* Canopy wetness state — must be reset between runs */
static int canopy_lastwet = 0;
static int canopy_wetsteps = 0;
void reset_canopy(void) { canopy_lastwet = 0; canopy_wetsteps = 0; storage = 0.0; ea = 0.0; es = 0.0; VPD = 0.0; }
double Rnet_absorb;
		...
		double *soilevaporation)
{
	...
	extern void topout (int tstep);
	/* use module-level state so it can be reset between runs */
#define lastwet  canopy_lastwet
#define wetsteps canopy_wetsteps
	...
	rra = ra (Z, z_0, d, data[id.win].xy[tstep].y);
      if (calcrs) { /* we have to estimate it using a script callback */
