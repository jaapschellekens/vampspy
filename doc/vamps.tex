...
\newcommand{\vampsversion}{{\tt 1.0}}
\newcommand{\vampsstatus}{{\tt Stable}}
...
\date{Version \vampsversion\ (\vampsstatus{}) --- April 2026}
...
\section*{Note on version 1.0}

Since version~1.0 the S-Lang scripting engine has been replaced by embedded
CPython.  User scripts are now plain Python~3 files hooked in via
\texttt{[vamps] xtrapy = myscript.py}.  The S-Lang intrinsics described
in the remainder of this manual are no longer available.

A Python package \texttt{vampspy} is provided for driving \vamps{} entirely
from Python --- no subprocess invocation, no temporary files.  It supports
single-column runs (via \texttt{Model.run()} or \texttt{Model.run\_stepwise()})
and parallel multi-column / 2-D grid runs (\texttt{Model.run\_grid()}).
See \texttt{docs/vampspy\_interface.md} in the source tree for the full API
reference and \texttt{notebooks/} for worked examples.

Source and issue tracker: \url{https://github.com/jaapschellekens/vampspy}
{\small
\begin{verbatim}
