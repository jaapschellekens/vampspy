...
\paragraph{Brooks and Corey}
\citeasnoun{brookscorey1964} proposed a power-law description of the
soil moisture characteristic that uses an explicit air-entry (bubbling)
pressure head $h_b$ below which the soil begins to desaturate.  The
effective saturation $S_e$ is defined as:
...
The retention curve and unsaturated conductivity are then given by:

%\begin{equation}
%S_e(h) = \left\{ \begin{array}{ll}
%    \left(\dfrac{h_b}{h}\right)^{\lambda} & h < h_b \\[6pt]
%    1 & h \geq h_b
%    \end{array} \right.
%\end{equation}
...
$h_b$       & air-entry (bubbling) pressure head ($h_b < 0$) \\
...
Starting approximations for converting van Genuchten parameters to
Brooks-Corey are $\lambda \approx n_\mathrm{vG} - 1$ and
$h_b \approx -1/\alpha$.  The two retention curves agree closely for
$S_e \leq 0.80$; the main difference is near saturation where Brooks-Corey
has a discrete air-entry while van Genuchten desaturates smoothly.
